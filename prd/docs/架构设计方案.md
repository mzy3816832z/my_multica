...
> 变更记录（YUZ-232，冒烟测试问题修复）：
> 1. **房型「朝向(orientation)」字段已删除**——该字段曾在 V1.1 迭代（F3）新增，现按产品要求移除：发布/编辑表单、房型数据模型、房源对比、接口序列化同步删除；朝向系统字典保留但不再被表单使用。
> 2. **「热门区域搜索」功能已删除**——移除搜索弹窗与空结果态的热门区域快捷词，及后端 `GET /api/v1/apartments/hot-districts` 接口。
> 3. **「可入住时间」日期选择组件修复**——原生 `input[type=date]` 点击不拉起日期选择，改为 Vant 日历/日期选择器。
> 4. **「面积排序」已删除**——面积跟随房型（一个房源可有多个房型），按面积排序存在歧义，故移除列表排序中的「面积从大到小/面积从小到大」选项及后端 `area_desc/area_asc` 排序口径；`min_area` 缓存字段保留用于详情/对比展示。
...
│   ├── config.py               # 环境变量与配置
│   ├── database.py             # SQLAlchemy engine/session/基础模型
│   ├── dependencies.py         # JWT 鉴权依赖、当前用户、权限检查
│   ├── exceptions.py           # 业务异常与统一异常处理器
│   ├── models/                 # ORM 模型
│   │   ├── user.py
│   │   ├── apartment.py
│   │   ├── room_type.py
...
| ~~orientation~~ | ~~VARCHAR~~ | ~~30~~ | ~~是~~ | ~~-~~ | ~~朝向编码~~（已删除，见变更记录 YUZ-232） |
| floor | INT | - | 是 | - | 楼层 |
| sort | INT | - | 是 | 0 | 展示排序 |
| created_at | TIMESTAMPTZ | - | 是 | now() | - |
| updated_at | TIMESTAMPTZ | - | 是 | now() | - |
...
| type | VARCHAR | 30 | - | - | first_rejected / change_rejected |
| title | VARCHAR | 100 | 是 | - | 标题 |
| content | TEXT | - | 是 | - | 内容 |
| related_apartment_id | BIGINT | - | 是 | - | 关联房源 |
| related_audit_id | BIGINT | - | 是 | - | 关联审核单 |
